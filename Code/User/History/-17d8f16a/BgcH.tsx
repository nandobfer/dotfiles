...
export const StatusLogs: React.FC<StatusLogsProps> = ({ logs }) => {
    return (
        ...
        >
            Atividade
            {logs.map((log) => (
                <Box key={log.id} sx={{ width: "20w", fontSize: "0.75vw", gap: "0.5vw", alignItems: "center", color: "text.secondary" }}>
                    <Circle status={log.status} />
                    <Box>{log.user.name.split(" ")[0]}</Box>
                    <Box sx={{ width: "10vw", marginLeft: "auto" }}>{new Date(log.datetime).toLocaleString("pt-br")}</Box>
                </Box>
            ))}
        </Paper>
    )
