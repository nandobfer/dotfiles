...
export const Contracts: React.FC<ContractsProps> = ({}) => {
    ...
    const handleSearch = (event: string | React.ChangeEvent<any>) => {
        const searchResult = contracts.list.filter(contract)
    }
    ...
    return (
        <div className="Contracts-Component">
            <SearchField onChange={handleChange} loading={contracts.loading} />
            {contracts.loading ? (
