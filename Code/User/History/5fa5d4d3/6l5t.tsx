...
introdução: The highlight of this project was the development time. We got the first build up and running in just a couple weeks and the final version came out within 2 months, which is very impressive development wise. A project this large takes, generally, 12 months to write.

1) We got the project with basically just the visual identity and the app's idea which was an app for a mall's store with only one product of each in it's shelters. And a Qr Code for each one. The customer has a virtual cart in the app and adds products to it by scanning the Qr code and proceeds to shipping (if he wants) and then to payment. All in the app. We ended up deploying 4 applications. A mobile (for customer's own devices), a tablet and totem (which are common devices) and an administration panel designed to manage products, categories, suppliers and orders.

2)  We built everything from scratch, from the design for all four applications, to code base which was entirely written for this project, it's original, to infrastructure that handles the database, an api also written from the ground, and finally setting everything up hosting it all in our server.
We chose to write this project's code in React.js which is a javascript framework developed by Facebook. It's a pretty modern technology and extremely performance oriented. 