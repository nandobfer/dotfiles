...
export const buildWorkedHours = (logs: StatusLog[]) => {
    ...
    const connectedTimes = days.map((logs) => {
        ...
        const totalMinutes = logs.reduce(
            (acc, log, index) => {
                ...
                if ((status === 0 || index === logs.length - 1) && acc.connectedStartTime) {
        ...
        return { date: logs[0].datetime, minutes: totalMinutes.totalMinutes }
    })
    ...
    return { data: connectedTimes.sort((a, b) => new Date(a.date).getTime() + new Date(b.date).getTime()), columns }
}
