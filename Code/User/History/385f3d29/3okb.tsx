...
export const Sellers: React.FC<SellersProps> = ({}) => {
    ...
    const [sellerList, setSellerList] = useState<User[]>(sellers)
    ...
    const onSearch = (value: string) => {
        const searchResult = sellers.filter((seller) => seller.name.toLowerCase().includes(value))
        setSellerList(searchResult)
    }

    useEffect(() => {
        if (sellerList.length == sellers.length) {
    ...
    return (
        <div className="Sellers-Component">
            <SearchField onChange={onSearch} loading={loading} />
            <p>Administradores</p>
