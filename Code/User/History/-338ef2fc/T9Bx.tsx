...
export const CustomerContainer: React.FC<CustomerContainerProps> = ({ customer }) => {
    ...
    return (
        ...
        >
            ...
            <Box sx={{ flexDirection: "column", gap: "1.5vw", height: "20vw" }}>
                <Box sx={{ gap: "10vw", alignItems: "center" }}>
                ...
                <Box sx={{ gap: "0.5vw", width: "30vw", flexWrap: "wrap", height: "5vw" }}>
                    {customer.services.map((service) => (
                        <Tag
                            key={service.id}
                            name={service.tag}
                            tooltip={service.name}
                            sx={{ fontSize: "0.7vw", padding: "0.25vw 0.5vw", height: "1.5vw" }}
                            color={customer.active ? "" : theme.palette.error.main}
                        />
                    ))}
                </Box>
                <p style={{ color: colors.text.secondary, fontSize: "0.75vw", width: "30vw", textAlign: "justify" }}>{customer.recomendations}</p>
            </Box>
        </Box>
    )
}
