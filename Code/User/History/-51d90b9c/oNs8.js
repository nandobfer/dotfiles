...
router.post("/confirm", async (request, response, next) => {
    ...
    if (data.signing == "sion") {
        ...
        if (
            ...
            console.log({
                contract: { birth: new Date(contract.seller.birth).getTime(), name: contract.seller.name.trim().toLowerCase() },
                confirm: { birth: data.birth, name: data.name.trim().toLowerCase() },
            })
        contract = null
