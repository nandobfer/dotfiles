                    ...
                    {({handleChange}) => (
                        <Form>
                            ...
                            <InputField className='contact-input' multiline inputProps={{height: '6vw'}} id='message' title={'Mensagem'} handleChange={handleChange} />
                                <div>
                                    {mailIsSent ? "Mensagem enviada" : <Button variant='contained' className='contact-send-button' type='submit'>{loading ? <CircularProgress size='1.5rem' sx={{color: 'white'}} /> : "Enviar"}</Button>}
                                </div>
                        </Form>
                    )}
                </Formik>
