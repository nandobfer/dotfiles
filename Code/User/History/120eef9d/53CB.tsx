...
import { Box, Paper, Skeleton, SxProps, alpha, lighten } from "@mui/material"
...
export const Profile: React.FC<ProfileProps> = ({ user, admin, createOnly }) => {
    ...
    return (
        <Box sx={{ ...backgroundStyle, height: "115vh", overflow: "hidden" }}>
            {!admin && <Header user={user} disabledSearch />}
            <Box sx={{ padding: "2vw", height: "79%", overflow: "hidden" }}>
                <Paper
                    elevation={3}
                    sx={{
                        width: "100%",
