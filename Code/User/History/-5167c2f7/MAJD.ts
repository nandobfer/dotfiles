    ...
    crops: {
        ...
        categories: {
            list: (callback: (categories: Category[]) => void) =>
                prisma.category.findMany({ include: { crops: true } }).then((result) => callback(result)),
        },
    },
    chats: {
    ...
    business: {
        ...
        categories: {
            list: (callback: (categories: BusinessCategory[]) => void) =>
                prisma.businessCategory.findMany({ include: { businesses: true } }).then((result) => callback(result)),
            add: (data: { name: string }, callback: (category: BusinessCategory) => void) =>
                prisma.businessCategory
                    .create({
                        data: { name: data.name },
                    })
