...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ external }) => {
    ...
    const [currentLocation, setCurrentLocation] = useState<
        ...
        | undefined
    >(external ? undefined : currentSection.navigation!.filter((item) => pathname == item.location)[0])

    useEffect(() => {
        console.log(currentSection.navigation!.filter((item) => pathname == item.location))
        if (!firstRender && currentLocation) {
            navigate(`/${currentSection.location}${currentLocation ? "/" : ""}${currentLocation.location}`)
        } else {
